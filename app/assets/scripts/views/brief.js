...
import ChartMultiple from '../components/chart-multiple'
...
export class Brief extends React.Component {
  ...
  addCharts (charts, scenarios) {
    _.forEach(charts, (data, name) => {
      ...
      if (placeholder) {
        if (data.mark === 'line') {
          ReactDOM.render(<ChartMultiple name={name} data={data} scenarios={scenarios} updateChart={this.updateChart}/>, placeholder)
        } else {
